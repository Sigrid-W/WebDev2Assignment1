...
for (let i = 0; i < characters.length; i++) {
  ...
  nameList.innerHTML += "<li>" + characters[i].name + "</li>";
}
...
for (let i = 0; i < characters.length; i++) {
  if (characters[i].age < 40) {
    console.log(characters[i].name);
    youngCharactersList.innerHTML += "<li>" + characters[i].name + "</li>";
  }
}
...
for (let i = 0; i < characters.length; i++) {
  if (characters[i].name) {
    functionList.innerHTML += "<li>" + characters[i].name + "</li>";
  }
}
...
for (let i = 0; i < characters.length; i++) {
  if (characters[i].name && characters[i].age < 40) {
    ageFilterList.innerHTML += "<li>" + characters[i].name + "</li>";
  }
}
...
for (let i = 0; i < characters.length; i++) {
  if (!characters[i].name) {
    let errorMsg = "Error: Character with id " + characters[i].id + " is missing a name.";
    if (errorMessages) {
      errorMessages.innerHTML += "<div class='error-message'>" + errorMsg + "</div>";
    }
  } else {
    errorHandlingList.innerHTML += "<li>" + characters[i].name + "</li>";
  }
}
...
const brokenCharacters = [
  { id: 11, age: 50 },
  { id: 12, age: 34 },
  { id: 13, name: "Harry Potter", age: 36 },
  { id: 14 }
];
let brokenArrayList = document.getElementById("broken-array-list");
let brokenArrayErrors = document.getElementById("broken-array-errors");
for (let i = 0; i < brokenCharacters.length; i++) {
  if (!brokenCharacters[i].name) {
    let errorMsg = "Error: Character with id " + brokenCharacters[i].id + " is missing a name.";
    if (brokenArrayErrors) {
      brokenArrayErrors.innerHTML += "<div class='error-message'>" + errorMsg + "</div>";
    }
  } else {
    brokenArrayList.innerHTML += "<li>" + brokenCharacters[i].name + "</li>";
  }
}
